...
const Share = ({setPosts , user}) => {
    ...
    const upload =  (image) => {
        ...
        const uploadedTask = storage.ref(`/facebook_Post_Videos/${fileName}`).put(image)
        ...
        ("state_changed",
            (snapshot) => {
                ...
                console.log(`Video is ${progress} % done`)
        },
            (err) => {console.log(err)},
            () => {
                uploadedTask.snapshot.ref.getDownloadURL().then((url) => {
                    setPostData((prev) => { return { ...prev , video: url } })
                    setPostData((prev) => { return { ...prev , type: "video" } })
                })
            }
        )
    }
                ...
                <Box className={classes.firstPort} >
                    ...
                    <FormControl sx={{ m: 1 }} variant="standard" style={{maxWidth: '520px'}} >
                        <TextField
                            ...
                            label="Want to Post a video content ..."
                ...
                <Box className={classes.postIcons} >
                    <IconButton
                        ...
                        <input type="file" class="form-control-file" id="exampleFormControlFile1" style={{display:'none'}}  
                            onChange={(e) => upload(e.target.files[0]) } accept="video/*"
                        />
                    </IconButton>
                    <IconButton
                        size="small"
